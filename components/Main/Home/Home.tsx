...
const HomeFix: React.FC<Props> = ({navigation}) => {
  ...
  const waterNotifLottieRef = useRef<LottieView>(null);
  const fertNotifLottieRef = useRef<LottieView>(null);
  ...
  const isBlock1WaterActive = useMemo(() => {
    return (
  ...
  const isBlock1FertilizerActive = useMemo(() => {
    return (
  ...
  // useEffect(() => {
  //   if (waterNotifLottieRef.current) {
  //     if (isBlock1WaterActive) {
  //       waterNotifLottieRef.current.play();
  //     } else {
  //       waterNotifLottieRef.current.pause();
  //     }
  //   }

  //   if (fertNotifLottieRef.current) {
  //     if (isBlock1FertilizerActive) {
  //       fertNotifLottieRef.current.play();
  //     } else {
  //       fertNotifLottieRef.current.pause();
  //     }
  //   }
  // }, [isBlock1WaterActive, isBlock1FertilizerActive]);
...
// Remove the existing standalone useEffect for BackHandler since it's now in useFocusEffect
  ...
  const handleToggle = useCallback(
    (type: 'water' | 'fertilizer') => {
      // Cek jika di Detail Block One ada proses apa pun
                ...
                <CornerCutComponent
                  ...
                  <View style={styles.cardBlockContent}>
                    <View style={styles.containerVector}>
                      ...
                      {/* {isBlock1FertilizerActive && (
                        <View style={styles.notifIconFertilizer}>
                          <LottieView
                            ref={fertNotifLottieRef}
                            source={require('../../../assets/videos/pupuk.mp4.lottie.json')}
