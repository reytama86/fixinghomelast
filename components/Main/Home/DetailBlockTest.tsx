...
const DetailBlockOne: React.FC<Props> = ({ navigation }) => {
  ...
  const fetchSensorData = async (): Promise<void> => {
    try {
      setLoading(true);
      const response = await fetch('http://10.0.2.2:4646/api/latest-sensor-block1');
  ...
  // Komponen untuk Device 1 (Sensor 1)
  const Device1 = () => {
  ...
  // Komponen untuk Device 2 (Sensor 2)
  const Device2 = () => {
  ...
  // Komponen untuk Device 3 (Sensor 3)
  const Device3 = () => {
...
const styles = StyleSheet.create({
  ...
  gridRow: {
    flexDirection: 'row',
    gap: 8,
    height: 44, // Reduced height to fit 4 rows in available space
  },
