...
const ExpandableBlock = React.memo<{
  ...
  ({
    ...
    const isMainActive = useMemo(() => {
      return blockType === 'water' ? mainControl.isWaterOn : mainControl.isFertilizerOn;
    ...
    const handleToggle = useCallback(
      (target: 'main' | 'row1' | 'row2') => {
        ...
        if (target === 'main' && isAnyRowActive) {
          Alert.alert('Cannot Start', 'Please stop row processes first before starting main control.');
          return;
        }
      ...
      [mainControl, row1Control, row2Control, blockType, isDisabled, isMainActive, isAnyRowActive],
    );
    ...
    const rows = useMemo(
      () => [
        {key: 'row1' as const, label: 'Baris 1 & Baris 5'},
    ...
    <View
      ...
      <View style={styles.infoSpraying}>
        ...
        {info[2] && (
          <Text style={[
            styles.infoTextBold,
            isInProgress && {fontFamily: 'SpaceGrotesk-Medium', color: 'black', fontSize: 12}
          ]}>
            {info[2]}
          </Text>
        )}
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={{fontFamily: 'SpaceGrotesk-Medium'}}>
          Loading sensor data...
        </Text>
      </View>
    );
